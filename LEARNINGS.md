...
Eight slots currently audited: `info-bar-bg`, `volume-icon`, `window-body-bg`, `desktop-bg`, `dialog-body-bg`, `info-bar-text-color`, `progress-bar-hue`, `folder-scene-icons`. New slot = one entry in the audit script + one section in the spec doc.
...
- **floppies + windows-95 default ring thin.** Bundle ships `-10231`/`-10232` (16×16) with 3px TRANSPARENT outer rim around the visible border. 9-slicing to a wider destination preserves the transparent rim, so the visible ring sits inset from the canvas. The kDEF model for these rings is likely 1:1 stamping at the artist's intended ring size, not scaling — contradicts our 9-slice-to-fit approach. Needs either a "tile-when-rim-transparent" detection or `resizeBehavior=anchor-*` honouring.
- **monkey-paradise OK button dark-wood face.** Bundle assigns `-10239` (the normal-push-button id slot) to "solo-menu-background" — non-canonical. The dark-wood button art is at `-10238` (pressed). Runtime correctly resolves `-10239` by id but gets the menu cicn. Either a per-theme remap or a "this -10239 is too small / wrong-aspect to be a button face" heuristic.
