...
Generated by `node scripts/extract-scheme.mjs <slug>` (which calls the extractor's `buildThemeJson` and validates via `validateTheme`). Always conformant to the schema in [`src/types.ts`](../src/types.ts). **Never hand-edit** — regenerate from `scheme.rsrc` (+ optional `meta.json`).
