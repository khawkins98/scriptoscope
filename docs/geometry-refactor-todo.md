...
| platinum-8 (+ any 8-bit icon, any scheme) | Finder icons (-3984 trash, 3967, -3985 …) | **TWO REAL BUGS** — blue-black blotches + checkerboard noise | **(1) PALETTE256 is WRONG.** `scripts/extract-icons.mjs`'s hardcoded 8-bit palette is a hand-built 6×6×6 cube + *my own* R/G/B/gray ramp arrangement at 216–255. It does NOT match the real Mac 256 CLUT: idx 0 ours=`[255,255,255]` real=`[0,0,0]`; idx 255 ours=`[17,17,17]` should be black; idx 245 ours=`[0,0,17]` (blue) where icons expect a gray. The rainbow Apple logo decoded fine ONLY because it uses the color CUBE (correct); icons using the ramp/gray region come out wrong. **(2) Missing masks** — these icons ship no ICN#/ics#, so the background renders opaque (compounding the palette garbage). | the AUTHORITATIVE Mac 256 system CLUT (sourced, not re-approximated) + masks | **PALETTE = FIXED ✅.** Found the real palette: Mac OS 8.5 System file `clut 9`, normalized ÷0x7fff (it was stored at 15-bit/half scale) → `scripts/lib/mac-system-palette.json`. Verified: idx0=white, idx255=black, idx245=gray (was blue); renders the rainbow Apple AND a clean grayscale trash can. extract-icons.mjs now loads it; all 8-bit icons re-decoded. The blue-black blotches are GONE. **Remaining: masks** (icons lacking ICN# still render an opaque background) — source from the System-file icon suite, or corner-flood / skip. |
