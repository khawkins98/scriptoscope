...
| 1138 (native kDEF) | document | reference seems to have a recessed INNER content-well border our render "lacks" → apparent content shift | **CONTENT, not chrome** (determined) | the `cicn -14335` frame art + the rendered chrome | **CLOSED — chrome is FAITHFUL.** Determined by inspection: the window cicn's frame art DOES carry the 1px inner content-well border (it bounds the cicn's content hole), and our 9-slice REPRODUCES it — verified the inner black line is present in the rendered left edge, and the content hole starts at x≈11 = `part-0 left` (frame width correct, NOT over-trimmed). So the EXTRA inset the reference shows is the Finder LIST-VIEW's own content border — part of the screenshot's *content*, which our generic-controls playground doesn't draw. Not a geometry bug. (If pixel-aligned content comparison is wanted, the playground would need to render Finder-like list content — a demo-content enhancement, not a chrome fix.) |
| platinum-8 / system7 / aplat2 | document | "corner-sprite polish" bucket: widgets too big (~13 vs ref ~10), platinum-8 bar washed-out, inner-bevel top pattern | all **FAITHFUL / already-addressed** (investigated, not patched) | shipped glyph ink · `cicn -14331` · the topBorder carve | **CLOSED — no reactive patches (punch-list-trap discipline).** (1) Widget size: we stamp the shipped 13×13 glyph 1:1; the reference only *looks* ~10px because that screenshot is downscaled ~0.77× (19px bar→~15). Faithful. (2) platinum-8 bar: re-confirmed FLAT by design — `-14331` is a single-colour block (vertical row-delta 0.0 vs aplat2 striped 105). Do NOT add stripes. (3) Inner-bevel top: the `topBorder` raised frame edge (c8964a8) + §2b raised bevel cover it; no reference evidence for more. |
