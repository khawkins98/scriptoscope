...
| platinum-8 (+ any scheme) | Finder icons (-3984 trash, 3967, -3985 …) | **REAL BUG** — deep blue-black blotches + checkerboard noise | platinum-8 ships 37 icl8 but only 3 ICN# masks; these icons have **NO mask** → `decodeIcon8` makes every pixel opaque → undefined background bytes render as garbage (e.g. index 245→[0,0,17]). Same defer-to-OS pattern, for MASKS: the scheme ships custom-colored Finder icons but relies on the OS's standard mask for that Apple-Icon-Services id. | the OS's standard mask for that id (we HAVE it — System file icon suite) | **OPEN, fixable.** Source the standard ICN#/ics# mask for these ids from `85-System.bin` (Apple Icon Services suite) + apply to maskless icl8/ics8. Fallbacks if no system mask: corner-flood-transparency (works for uniform bg, not noise) or skip rendering maskless icons. NOT a palette/decode bug — the color decode is correct. |
