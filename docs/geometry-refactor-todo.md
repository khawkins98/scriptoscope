...
| platinum-8 (+ any 8-bit icon, any scheme) | Finder icons (-3984 trash, 3967, -3985 …) | **TWO REAL BUGS** — blue-black blotches + checkerboard noise | **(1) PALETTE256 is WRONG.** `scripts/extract-icons.mjs`'s hardcoded 8-bit palette is a hand-built 6×6×6 cube + *my own* R/G/B/gray ramp arrangement at 216–255. It does NOT match the real Mac 256 CLUT: idx 0 ours=`[255,255,255]` real=`[0,0,0]`; idx 255 ours=`[17,17,17]` should be black; idx 245 ours=`[0,0,17]` (blue) where icons expect a gray. The rainbow Apple logo decoded fine ONLY because it uses the color CUBE (correct); icons using the ramp/gray region come out wrong. **(2) Missing masks** — these icons ship no ICN#/ics#, so the background renders opaque (compounding the palette garbage). | the AUTHORITATIVE Mac 256 system CLUT (NOT a re-approximation — guessed wrong twice) + the standard masks (System-file icon suite) | **OPEN — TOP PRIORITY, confirmed ours.** Fix: (a) replace PALETTE256 with the real Mac 8-bit system CLUT sourced from a real `clut` resource / verified table; (b) source/derive masks. Then re-extract. ⚠️ Earlier "color decode is correct" was WRONG — corrected here. |
