...
| apple-platinum-2 | controls (radio etc.) | icons look flatter / "shades missing" vs reference | **NOT a decode/palette bug** — verified: ics8 radio = 12 shades (exact Mac 17-step gray ramp + indigo dot); PALETTE256 is the real Mac palette (clut-9 was a red herring, not the icon palette). | the scheme's own ics8 (faithfully decoded at full depth) | **CLOSED — recreation art ceiling.** apple-platinum-2 (1999) drew a gray+indigo radio; Apple's real Platinum is a blue gradient sphere. The gap is ART, not depth. True Apple-gradient fidelity = the Appearance Manager `.thme` STRETCH goal. |
