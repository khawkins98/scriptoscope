...
A web-native runtime that renders classic [Kaleidoscope](https://en.wikipedia.org/wiki/Kaleidoscope_(software)) themes pixel-for-pixel in the browser, from the original binary resources their authors shipped. Drop in a `.sit` and the same `cicn` artwork, `wnd#` rect lists, and `cinf` color tables that drove the 1999 control panel drive the renderer here. The window-chrome compositor is a clean-room reimplementation of the decompiled Kaleidoscope **2.3.1** kDEF — same part-code jump table, same recipe walk — so getting the engine right once renders every scheme. Eighteen are bundled; any of the 3,000-plus on [kaleidoscope.hryjksn.com](https://kaleidoscope.hryjksn.com/) will load from the demo's drop zone.

This is a passion project. I grew up in the nineties — refreshing the Kaleidoscope.net scheme folder on a 56k modem, running *Apple Lisa for K2.1* because it looked like the first computer I touched, swapping *Mod* and *BeOS R5* and *Crayon OS* the way other people swapped wallpapers. When OS X arrived the format went quiet, and a couple of decades later it occurred to me that none of that work should be unreachable just because the runtime that played it is. So: web-native, MIT, no original code shipped or executed — an API-compatible mimic of the binary's rendering model, reverse-engineered from the decompile and cross-checked against reference renders. I emailed Greg Landweber and Arlo Rose and a few of the scheme authors I could find addresses for; if any of them ever reply, that'll be the best day of this project.

If you're crazy enough to try it out — port one of your old schemes, drop the runtime onto a personal site, [open an issue](https://github.com/khawkins98/aaron-ui/issues) — please let me know. And if you ever wrote a Kaleidoscope scheme, [the door's open](#if-you-build-a-scheme).
